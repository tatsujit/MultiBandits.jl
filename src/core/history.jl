...
全履歴型の基底抽象型。シミュレーション中の行動・報酬・期待値を記録する。
"""
...
定常環境用の履歴。各試行の行動・報酬と、環境の期待値を記録する。

# フィールド
- `n_arms::Int` — 腕の数
- `trials::Int` — 試行数
- `actions::Vector{Int}` — 各試行で選択された行動
- `expectations::Vector{Float64}` — 環境の期待報酬（定常なので共通）
- `rewards::Vector{Float64}` — 各試行で得た報酬
"""
mutable struct History <: AbstractHistory
    n_arms::Int
    trials::Int
    actions::Vector{Int}
    expectations::Vector{Float64}
    rewards::Vector{Float64}
    function History(n_arms::Int, trials::Int)
        actions = zeros(Int64, trials)
        # expectations = [zeros(n_arms) for _ in 1:trials] # これだと trial x n_arms のサイズで、大きくなりすぎる
        expectations = zeros(n_arms) # とりあえず１つだけ入れておく
...
# dataframe() functions are provided by MultiBanditsDataExt when DataFrames is loaded
...
function record!(history::History, trial::Int, action::Int, expectations::Vector{Float64}, reward::Float64)
    ...
    # end
    if trial == 1
        history.expectations = expectations
    else
        @assert history.expectations == expectations "expectations are not the same; non-stationary environment is not supported"
    end
...
"""
    EstimatorHistory <: AbstractHistory

`CognitiveEstimator` の内部状態（Q, V, W, C, N, 選択確率）を毎試行記録する拡張履歴。

# フィールド
- `Qss::Matrix{Float64}` — 各試行 × 各腕の Q 値
- `Vss::Matrix{Float64}` — 各試行 × 各腕の効用値 V
- `Wss::Matrix{Float64}` — 各試行 × 各腕の総合行動価値 W
- `Css::Matrix{Float64}` — 各試行 × 各腕の固執性 C
- `Pss::Matrix{Float64}` — 各試行 × 各腕の選択確率
"""
mutable struct EstimatorHistory <: AbstractHistory
    ...
    function EstimatorHistory(trials::Int, n_arms::Int, est::CognitiveEstimator)
        actions = zeros(Int64, trials)
...
function record!(history::EstimatorHistory, trial::Int, action::Int,
                 ...
                 est::CognitiveEstimator=nothing)
    n_arms = history.n_arms # length(expectations) #
    history.actions[trial] = action
    # @ic model
...
"""
    LAHistory <: AbstractHistory

Limited Availability (制限付き腕) 環境用の履歴。各試行で利用可能な腕のサブセットも記録する。
"""
