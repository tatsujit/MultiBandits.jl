...
function to_estimator(config::EstimatorConfig, n_arms::Int)
    # Check if CognitiveEstimator is defined
    if !@isdefined(CognitiveEstimator)
        error("CognitiveEstimator struct not defined. Include cognitive_estimator.jl first.")
    end

    # For betaCDF utility function, we let the CognitiveEstimator constructor handle it
    if config.utility_function == "betaCDF" || config.betaCDF
        return CognitiveEstimator(
            n_arms;
            Q0 = config.Q0,
            α = config.α,
            αm = config.αm, 
